...
export async function generateExcelBlob(result: PipelineResult): Promise<Blob> {
  ...
  // 3. チェーン店・商業施設除外リスト
  const excluded = [
    ...(result?.chainExcluded || []),
    ...(result?.mallExcluded || []),
  ];
  const excludedData = excluded.map((r) => {
    ...
    return {
      ...
      ソース: r.source,
      該当チェーン名:
        (chainLog?.meta?.matchedChainName as string) ||
        (aeonLog ? 'イオンモール' : '') ||
        ((commLog?.meta?.matchedFacility as string) || ''),
      判定タイプ:
        (chainLog?.meta?.matchType as string) ||
        (aeonLog ? 'イオンモール除外' : '') ||
        (commLog ? '商業施設除外' : ''),
    };
  });
  const wsExcluded = XLSX.utils.json_to_sheet(excludedData);
  XLSX.utils.book_append_sheet(wb, wsExcluded, '除外リスト');
